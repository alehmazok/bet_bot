...
class TelegramUserModelTest(TestCase):
    ...
    def test_full_name_property(self):
        ...
        user_last_only = TelegramUserModel.objects.create(
            telegram_id=444555666, last_name="Brown"
        )
        self.assertEqual(user_last_only.full_name, "Brown")

        # Test with no names
        user_no_names = TelegramUserModel.objects.create(telegram_id=777888999)
    ...
    def test_unique_telegram_id(self):
        ...
        with self.assertRaises(IntegrityError):
            TelegramUserModel.objects.create(
                telegram_id=123456789, first_name="Duplicate"  # Same as existing
            )
...
class TelegramBotHandlersTest(TestCase):
    ...
    async def test_register_user_success(self):
        ...
        with patch("telegram_bot.bot.sync_to_async") as mock_sync_async:
            # Mock the sync_to_async wrapper to return an AsyncMock
            mock_user = Mock()
            mock_user.telegram_id = 123456789
            mock_sync_async.return_value = AsyncMock(return_value=(mock_user, True))
    ...
    async def test_register_user_exception(self):
        ...
        with patch("telegram_bot.bot.sync_to_async") as mock_sync_async:
            # Mock sync_to_async to return an AsyncMock that raises an exception
            mock_sync_async.return_value = AsyncMock(
    ...
    async def test_start_command(self):
        ...
        with patch(
            "telegram_bot.bot.register_user", new_callable=AsyncMock
        ) as mock_register, patch("telegram_bot.bot.get_template") as mock_template:
    ...
    async def test_schedule_command_with_games(self):
        ...
        # Create mock games
        mock_games = [Mock(spec=Game), Mock(spec=Game)]

        with patch(
            "telegram_bot.bot.register_user", new_callable=AsyncMock
        ) as mock_register, patch(
            "telegram_bot.bot.sync_to_async"
        ) as mock_sync_async, patch(
            "telegram_bot.bot.get_template"
        ) as mock_template:

            # Mock sync_to_async to return an AsyncMock
            mock_sync_async.return_value = AsyncMock(return_value=mock_games)
            mock_template_instance = Mock()
            ...
            self.mock_message.reply_text.assert_called_once_with(
                "Schedule message", parse_mode="Markdown"
            )
    ...
    async def test_schedule_command_no_games(self):
        ...
        with patch(
            ...
            # Mock sync_to_async to return an AsyncMock
            mock_sync_async.return_value = AsyncMock(return_value=[])
            mock_template_instance = Mock()
            ...
            mock_register.assert_called_once()
            self.mock_message.reply_text.assert_called_once_with(
                "No games message", parse_mode="Markdown"
            )
    ...
    async def test_users_command_page_1(self):
        ...
        with patch(
            ...
            # Create a side effect function that returns different AsyncMocks based on the function being called
            def mock_side_effect(func):
                if "get_users_list" in str(func):
                    return AsyncMock(return_value=(mock_users, 2))
                elif "get_user_stats" in str(func):
                    return AsyncMock(return_value=mock_stats)
                return AsyncMock()

            mock_sync_async.side_effect = mock_side_effect
            mock_template_instance = Mock()
            ...
            mock_register.assert_called_once()
            self.mock_message.reply_text.assert_called_once_with(
                "Users message", parse_mode="Markdown"
            )
    ...
    async def test_users_command_with_page(self):
        ...
        with patch(
            ...
            def mock_side_effect(func):
                if "get_users_list" in str(func):
                    return AsyncMock(return_value=(mock_users, 5))
                elif "get_user_stats" in str(func):
                    return AsyncMock(return_value=mock_stats)
                return AsyncMock()
            ...
            mock_register.assert_called_once()
            self.mock_message.reply_text.assert_called_once_with(
                "Users page 2", parse_mode="Markdown"
            )

    @pytest.mark.asyncio
    async def test_users_command_exception(self):
        """Test /users command with exception."""
        with patch("telegram_bot.bot.register_user") as mock_register, patch(
            "telegram_bot.bot.sync_to_async"
        ) as mock_sync_async:

            mock_sync_async.return_value = AsyncMock(
                side_effect=Exception("'Mock' object is not subscriptable")
            )
            ...
            mock_register.assert_called_once()
            self.mock_message.reply_text.assert_called_once_with(
                "❌ Error retrieving users list. Please try again later."
            )
...
class TelegramBotTemplatesTest(TestCase):
    ...
    def test_schedule_template(self):
        ...
        context = {"current_date": "15.01.2024", "games": [mock_game1, mock_game2]}
